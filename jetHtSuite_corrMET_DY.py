  ...
  for targetrate in rates:
      for objset,things in whats:
          ...
          for name,obj,col,msty,msiz in things:
              ...
              obj = obj.replace("$","")
              if obj=="Gen":
                  aGenMET, aGenMETphi, aGenMu1, aGenMu2 = makeGenMETArray(signal, obj, options.eta)
                  fGenMu1 = open("./"+odir+"/GenMu1.txt", 'w')
                  for i in range(0, len(aGenMu1)):
                      for j in range(len(aGenMu1[i])):
                          fGenMu1.write(str(aGenMu1[i][j])+' ')
                      fGenMu1.write('\n')
                  fGenMu1.close()
                  fGenMu2 = open("./"+odir+"/GenMu2.txt", 'w')
                  for i in range(0, len(aGenMu2)):
                      for j in range(len(aGenMu2[i])):
                          fGenMu2.write(str(aGenMu2[i][j])+' ')
                      fGenMu2.write('\n')
                  fGenMu2.close()
                  #fGenMu1 = open("./mets/GenMu1.txt", 'w')
                  fGen = open("./"+odir+"/GenMET.txt", 'w')
                  #for i in range(0, 1000):
                  for i in range(0, len(aGenMET)):
                      fGen.write(str(aGenMET[i])+" "+str(aGenMETphi[i])+'\n')
                  fGen.close()
              if obj!="Gen":
                  aRecoMET, aRecoMETphi = makeCorrMetArray(signal, obj, options.pt, options.eta, None)
                  fReco = open("./"+odir+"/"+obj+"MET.txt", 'w')
                  for i in range(0, len(aRecoMET)):
                      fReco.write(str(aRecoMET[i])+" "+str(aRecoMETphi[i])+'\n')
                  fReco.close()
              ...
              if obj!="Gen":
                  aCorrMET, aCorrMETphi = makeCorrMetArray(signal, obj, options.pt, options.eta, jecs)
                  fCo = open("./"+odir+"/Co"+obj+"MET.txt", 'w')
                  for i in range(0, len(aCorrMET)):
                      fCo.write(str(aCorrMET[i])+" "+str(aCorrMETphi[i])+'\n')
                  fCo.close()
              print(obj + " Corr MET")
              label = name
'''
